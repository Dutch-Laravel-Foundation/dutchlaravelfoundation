---
id: 201dbb24-5f6f-4c35-8de1-ba599424e035
blueprint: pages
title: 'Hosting Hotline'
template: templates/default
menu_color: white
call_to_action: 6bc4ea55-2728-4615-8d45-75d49d29d157
updated_by: 50ac328e-5084-4c6e-9551-2fb5bb9f3d88
updated_at: 1769073497
content:
  -
    type: heading
    attrs:
      level: 1
    content:
      -
        type: text
        text: 'Hosting Hotline!'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'De '
      -
        type: text
        marks:
          -
            type: bold
        text: 'Hosting Hotline'
      -
        type: text
        text: ' is een speciale service voor alle leden van de Dutch Laravel Foundation (DLF). De hotline is samen met onze partner Shock Media opgezet om leden te ondersteunen bij allerlei hosting gerelateerde vragen. Van eenvoudige issues tot complexe infrastructuur vraagstukken.'
      -
        type: hardBreak
  -
    type: paragraph
    content:
      -
        type: text
        text: 'Shock Media is onze partner op het gebied van hosting en infrastructuur. Dankzij hun jarenlange ervaring met managed hosting, serverbeheer en security kunnen we onze leden deze waardevolle service kosteloos aanbieden. Ook als je applicatie draait op de infrastructuur van een andere hoster.'
      -
        type: hardBreak
  -
    type: heading
    attrs:
      level: 2
    content:
      -
        type: text
        text: 'Hosting van applicaties kan complex zijn'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'Laravel applicaties en klanten kunnen specifieke eisen stellen op het gebied van performance, beveiliging en schaalbaarheid. Niet elke hoster of developer houdt standaard rekening met die complexiteit. De experts achter de hotline helpen je in dat geval graag op weg.'
      -
        type: hardBreak
  -
    type: heading
    attrs:
      level: 2
    content:
      -
        type: text
        text: 'Voorbeelden van vragen / problemen die je kunt insturen'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'Hieronder een aantal concrete voorbeelden van situaties of vragen waarvoor de Hosting Hotline geschikt is:'
  -
    type: bulletList
    content:
      -
        type: listItem
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'Problemen met serverconfiguratie: bijvoorbeeld fouten bij PHP-instellingen, onvoldoende geheugen of timeouts.'
      -
        type: listItem
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'Prestatieproblemen: traagheid van de applicatie, piekbelasting, load balancing, caching, database-optimalisatie, etc.'
      -
        type: listItem
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'Beveiliging & compliance: vragen over TLS/SSL, firewallregels, server hardening, of privacy/regelgeving omtrent hosting.'
      -
        type: listItem
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'Back-ups & herstel: advies of ondersteuning bij back-up strategieën, disaster recovery of data-herstel.'
      -
        type: listItem
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'Migraties en deployment: advies bij migreren van app of data tussen hosts, of het opzetten van een solide deployment-pipeline.'
      -
        type: listItem
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'Architectuurbeslissingen: bijvoorbeeld over shared hosting vs. VPS, container/cluster-opzet, schaalbaarheid of redundantie.'
  -
    type: paragraph
    content:
      -
        type: hardBreak
  -
    type: heading
    attrs:
      level: 2
    content:
      -
        type: text
        text: 'Stel dus nu je vraag'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'Heb je een dus een vraag over hosting, serverconfiguratie, beveiliging of infrastructuur? Of overweeg je een migratie of opschaling van je Laravel applicatie? Neem dan gerust contact op via de Hosting Hotline op '
      -
        type: text
        marks:
          -
            type: link
            attrs:
              href: 'mailto:hotline@shockmedia.nl'
              rel: null
              target: null
              title: null
        text: hotline@shockmedia.nl
      -
        type: text
        text: '. Wij helpen je graag verder.'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'Nog geen lid van de Dutch Laravel Foundation en dus nog geen gratis toegang tot de hotline? Bekijk dan onze informatie over lidmaatschap.'
  -
    type: paragraph
---
