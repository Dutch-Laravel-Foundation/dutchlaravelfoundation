...
it('every form email uses the branded laravel markdown layout', function () {
    $transport = Mail::mailer()->getSymfonyTransport();
    $transport->flush();

    $renderedEmails = 0;

    foreach (formSubmissions() as $formHandle => $submissionData) {
        ...
        foreach ($emailConfigs as $index => $emailConfig) {
            ...
            expect($emailConfig['markdown'] ?? false)->toBeTrue("{$notification} must use Laravel Markdown.");
            $this->assertArrayNotHasKey('text', $emailConfig);
            expect($emailConfig['html'] ?? null)->toBeString();
            ...
            expect($html)->toMatch('/<td class="footer-wrapper"[^>]*bgcolor="#ff2d20"[^>]*>/');
    ...
    expect($renderedEmails)->toBe(6);
    expect($transport->messages())->toHaveCount(6);
});
/**
 * @return array<string, array<string, mixed>>
 */
function formSubmissions(): array
{
    return [
        'become_member' => [
            'name' => 'Robin de Vries',
